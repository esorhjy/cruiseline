...
try {
  for (const viewport of [{width:1440,height:900},{width:390,height:844},{width:360,height:800}]) {
    ...
    await page.locator('[data-purpose="service"]').click();
    await page.locator('#deck-filter').selectOption('deck16');
    assert(await page.locator('#search-deck-deck16-laundry').isVisible());
    const deckOrder = await page.locator('#deck-filter option').evaluateAll(options => options.map(option => option.value));
    assert(deckOrder.indexOf('deck11') < deckOrder.indexOf('deck16') && deckOrder.indexOf('deck16') < deckOrder.indexOf('deck17'));
    await page.locator('#search-deck-deck16-laundry summary').click();
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), 'expanded laundry card fits mobile');
    await page.screenshot({path:path.join(output, `laundry-${viewport.width}.png`), animations:'disabled', timeout:15000});
    await page.locator('#search-deck-deck16-laundry [data-search-query]').click();
    await page.waitForSelector('.lookup-result-card');
    await page.locator('.lookup-crew-trigger').first().click();
    assert((await page.locator('.lookup-crew-card').textContent()).includes('Self-Service Laundry'));
    await page.keyboard.press('Escape');
    await page.locator('[data-search-tool-mode="guide"]').click();
    await page.locator('#search-input').fill('醫務中心');
    await page.waitForSelector('.search-result-card');
    await page.locator('.search-result-card').filter({hasText:'Health Center'}).first().click();
    await page.waitForSelector('#search-deck-deck9-health-center');
    assert(await page.locator('#search-deck-deck9-health-center').isVisible(), 'new service search navigates to primary card');
    await nav.locator('[data-view-link="prepare"]').click();
    assert.equal(await page.locator('.checklist-item[data-id="artwork-folder"] input').isChecked(), false, 'new folder task starts unchecked');
  ...
  if (!process.env.TEST_BASE_URL) {
    ...
    await offlinePage.goto(base + '#search-deck-deck16-laundry', {waitUntil:'domcontentloaded'});
    await offlinePage.waitForSelector('#search-deck-deck16-laundry');
    assert((await offlinePage.locator('#search-deck-deck16-laundry').textContent()).includes('Deck 16'));
    await offlinePage.locator('#nav-search-trigger').click();
    await offlinePage.locator('[data-search-tool-mode="guide"]').click();
    await offlinePage.locator('#search-input').fill('醫務中心');
    await offlinePage.waitForSelector('.search-result-card');
    assert((await offlinePage.locator('#search-results').textContent()).includes('Health Center'), 'new content searchable offline');
